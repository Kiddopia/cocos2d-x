...
{
    if (!MySprite::initWithTexture(pTexture, cocos2d::Rect(0, 0, rect.size.width, rect.size.height), rotated))
    {
        return false;
    }
    return true;
...
#endif
{
    return MySprite::draw(renderer, transform, flags);
}
...
#endif
{
    ...
    return MySprite::visit(renderer, transform, flags);
}
