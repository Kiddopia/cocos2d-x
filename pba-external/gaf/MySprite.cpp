...
MySprite* MySprite::createWithTexture(Texture2D *texture)
{
...
MySprite* MySprite::createWithTexture(Texture2D *texture, const Rect& rect, bool rotated)
{
    MySprite *sprite = new (std::nothrow) MySprite();
...
MySprite* MySprite::create(const PolygonInfo& info)
{
    MySprite *sprite = new (std::nothrow) MySprite();
...
MySprite* MySprite::createWithSpriteFrame(SpriteFrame *spriteFrame)
{
    MySprite *sprite = new (std::nothrow) MySprite();
...
MySprite* MySprite::createWithSpriteFrameName(const std::string& spriteFrameName)
{
...
bool MySprite::initWithTexture(Texture2D *texture, const Rect& rect)
{
    return initWithTexture(texture, rect, false);
}

bool MySprite::initWithFile(const std::string& filename)
{
    if (filename.empty())
    {
        CCLOG("Call MySprite::initWithFile with blank resource filename.");
        return false;
    }
...
bool MySprite::initWithFile(const std::string &filename, const Rect& rect)
{
...
bool MySprite::initWithSpriteFrameName(const std::string& spriteFrameName)
{
...
bool MySprite::initWithSpriteFrame(SpriteFrame *spriteFrame)
{
...
bool MySprite::initWithPolygon(const cocos2d::PolygonInfo &info)
{
    bool ret = false;
    ...
    {
        _polyInfo = info;
        setContentSize(_polyInfo.getRect().size / _director->getContentScaleFactor());
        ret = true;
    }
...
bool MySprite::initWithTexture(Texture2D *texture, const Rect& rect, bool rotated)
{
    bool result = false;
    if (Node::init())
    {
 ...
 *
 *   auto sp = new (std::nothrow) MySprite();
 ...
 *
 * The test is in "TestCpp/SpriteTest/MySprite without texture".
 */
...
// MARK: texture
void MySprite::setTexture(const std::string &filename)
{
...
void MySprite::setTexture(Texture2D *texture)
{
...
Texture2D* MySprite::getTexture() const
{
    return _texture;
}

void MySprite::setTextureRect(const Rect& rect)
{
    setTextureRect(rect, false, rect.size);
}

void MySprite::setTextureRect(const Rect& rect, bool rotated, const Size& untrimmedSize)
{
...
void MySprite::setVertexRect(const Rect& rect)
{
    _rect = rect;
}

void MySprite::setTextureCoords(const Rect& rectInPoint)
{
...
void MySprite::updateTransform(void)
{
    CCASSERT(_batchNode, "updateTransform is only valid when MySprite is being rendered using an SpriteBatchNode");
    
    // recalculate matrix only if it is dirty
    if( isDirty() ) {
        
        // If it is not visible, or one of its ancestors is not visible, then do nothing:
        if( !_visible || ( _parent && _parent != _batchNode && static_cast<MySprite*>(_parent)->_shouldBeHidden) )
        {
        ...
        else
        {
            ...
            {
                CCASSERT( dynamic_cast<MySprite*>(_parent), "Logic error in MySprite. Parent must be a MySprite");
                const Mat4 &nodeToParent = getNodeToParentTransform();
                Mat4 &parentTransform = static_cast<MySprite*>(_parent)->_transformToBatch;
    ...
    /*    if( _hasChildren )
     ...
     arrayMakeObjectsPerformSelector(_children, updateTransform, MySprite*);
     }*/
...
void MySprite::addChild(Node *child, int zOrder, int tag)
{
...
void MySprite::addChild(Node *child, int zOrder, const std::string &name)
{
    CCASSERT(child != nullptr, "Argument must be non-nullptr");
    if (child == nullptr)
    {
...
void MySprite::reorderChild(Node *child, int zOrder)
{
...
void MySprite::removeChild(Node *child, bool cleanup)
{
    if (_batchNode)
    {
...
void MySprite::removeAllChildrenWithCleanup(bool cleanup)
{
    if (_batchNode)
    {
...
void MySprite::setReorderChildDirtyRecursively(void)
{
    ...
    if ( ! _reorderChildDirty )
    {
        ...
        {
            static_cast<MySprite*>(node)->setReorderChildDirtyRecursively();
            node=node->getParent();
        }
    }
}

void MySprite::setDirtyRecursively(bool bValue)
{
    _recursiveDirty = bValue;
    setDirty(bValue);
    
    for(const auto &child: _children) {
        MySprite* sp = dynamic_cast<MySprite*>(child);
        if (sp)
        {
...
void MySprite::setPosition(const Vec2& pos)
{
...
void MySprite::setRotation(float rotation)
{
...
void MySprite::setRotationSkewX(float fRotationX)
{
    Node::setRotationSkewX(fRotationX);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setRotationSkewY(float fRotationY)
{
    Node::setRotationSkewY(fRotationY);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setSkewX(float sx)
{
    Node::setSkewX(sx);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setSkewY(float sy)
{
    Node::setSkewY(sy);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setScaleX(float scaleX)
{
    Node::setScaleX(scaleX);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setScaleY(float scaleY)
{
    Node::setScaleY(scaleY);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setScale(float fScale)
{
    Node::setScale(fScale);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setScale(float scaleX, float scaleY)
{
    Node::setScale(scaleX, scaleY);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setPositionZ(float fVertexZ)
{
    Node::setPositionZ(fVertexZ);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setAnchorPoint(const Vec2& anchor)
{
    Node::setAnchorPoint(anchor);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setIgnoreAnchorPointForPosition(bool value)
{
    CCASSERT(! _batchNode, "setIgnoreAnchorPointForPosition is invalid in MySprite");
    Node::setIgnoreAnchorPointForPosition(value);
}

void MySprite::setVisible(bool bVisible)
{
    Node::setVisible(bVisible);
    SET_DIRTY_RECURSIVELY();
}

void MySprite::setFlippedX(bool flippedX)
{
...
bool MySprite::isFlippedX(void) const
{
    return _flippedX;
}

void MySprite::setFlippedY(bool flippedY)
{
...
bool MySprite::isFlippedY(void) const
{
    return _flippedY;
}
...
void MySprite::updateColor(void)
{
...
void MySprite::setOpacityModifyRGB(bool modify)
{
...
bool MySprite::isOpacityModifyRGB(void) const
{
...
void MySprite::setSpriteFrame(const std::string &spriteFrameName)
{
...
void MySprite::setSpriteFrame(SpriteFrame *spriteFrame)
{
...
void MySprite::setDisplayFrameWithAnimationName(const std::string& animationName, ssize_t frameIndex)
{
...
bool MySprite::isFrameDisplayed(SpriteFrame *frame) const
{
...
SpriteFrame* MySprite::getSpriteFrame() const
{
...
SpriteBatchNode* MySprite::getBatchNode() const
{
    return _batchNode;
}

void MySprite::setBatchNode(SpriteBatchNode *spriteBatchNode)
{
...
void MySprite::updateBlendFunc(void)
{
...
std::string MySprite::getDescription() const
{
    int texture_id = -1;
    if( _batchNode )
    ...
    return StringUtils::format("<MySprite | Tag = %d, TextureID = %d>", _tag, texture_id );
}

const PolygonInfo& MySprite::getPolygonInfo() const
{
    return _polyInfo;
}

void MySprite::setPolygonInfo(const PolygonInfo& info)
{
    _polyInfo = info;
}
