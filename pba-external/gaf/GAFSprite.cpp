...
bool GAFSprite::initWithSpriteFrame(cocos2d::SpriteFrame *spriteFrame)
{
    ...
    bool bRet = MySprite::initWithTexture(spriteFrame->getTexture(), spriteFrame->getRect());
...
{
    if (MySprite::initWithTexture(pTexture, rect, rotated))
    {
...
void GAFSprite::setTexture(cocos2d::Texture2D *texture)
{
    ...
    CCAssert(!_batchNode || texture->getName() == _batchNode->getTexture()->getName(), "MySprite: Batched sprites should use the same texture as the batchnode");
...
void GAFSprite::setTextureRect(const cocos2d::Rect& rect, bool rotated, const cocos2d::Size& untrimmedSize)
{
    ...
    MySprite::setTextureRect(rect, rotated, rotatedSize);
}

void GAFSprite::setTextureCoords(const cocos2d::Rect& rect1)
{
    cocos2d::Rect rect = CC_RECT_POINTS_TO_PIXELS(rect1);
...
{
    CCAssert(!_batchNode, "If MySprite is being rendered by CCSpriteBatchNode, MySprite#draw SHOULD NOT be called");
